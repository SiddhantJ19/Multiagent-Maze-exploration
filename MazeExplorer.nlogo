...
turtles-own [stack visited]
patches-own [parent-patch]
...
to-report get-color [char]
  ...
  if char = "B" [report blue]
  if char = "E" [report red]
end
...
to init-turtles-at-source
  ...
  ask turtles [
    ...
    ask patch-here [set parent-patch NoBody]
  ]
end
...
to go
  path-finder-basic
  if [pcolor] of one-of turtles = red [stop]
  tick
end
